...
// Parse image string → array of full URLs
const parseImgs = (str='') =>
    str.split(',').map(s=>s.trim()).filter(Boolean).map(p =>
        p.startsWith('http') ? p : p.startsWith('/') ? `${BASE}${p}` : `${BASE}/images/${p}`
    );

const CATEGORIES = ["Tất cả","Thu hoạch","Nông trại","Kiểm định","Đóng gói","Vận chuyển"];
const CAT_ICON   = { "Thu hoạch":<Sprout size={11}/>, "Nông trại":<Building2 size={11}/>, "Kiểm định":<FlaskConical size={11}/>, "Đóng gói":<Package size={11}/>, "Vận chuyển":<Truck size={11}/> };
const CAT_COLOR  = { "Thu hoạch":'#22c55e', "Nông trại":'#38bdf8', "Kiểm định":'#818cf8', "Đóng gói":'#fbbf24', "Vận chuyển":'#f97316' };
...
export default function GalleryPage() {
    ...
    // ── Load: Farm → Batches → Products (đúng luồng kiến trúc) ──
    const loadImages = async () => {
        if (!user) return;
        setLoading(true);
        try {
            // 1. Lấy farms của farmer
            const farmRes = await farmService.getAll();
            const myFarms = (farmRes.data||[]).filter(f => f.OwnerId === user.id);
            const farmMap = Object.fromEntries(myFarms.map(f=>[f.id, f.FarmName||f.id]));

            // 2. Lấy batches theo từng farm
            const batchResults = await Promise.all(
                myFarms.map(f => batchService.getAll({ farmid: f.id }).catch(()=>({ data:[] })))
            );
            const allBatches = batchResults.flatMap(r => r.data||[]);

            // 3. Nhóm theo productId (loại bỏ trùng lặp)
            const productIds = [...new Set(allBatches.map(b=>b.productid).filter(Boolean))];

            // 4. Lấy thông tin products
            const productRes = await productService.getAll();
            const allProducts = productRes.data||[];

            // 5. Build image list từ products thuộc farms này
            const imgs = [];

            // Ảnh từ products (Farm → Batch → Product)
            productIds.forEach(pid => {
                const product = allProducts.find(p => p.id===pid || p._id===pid);
                if (!product?.image) return;
                const batch   = allBatches.find(b => b.productid===pid);
                const farmName = farmMap[batch?.farmid] || batch?.farmid || '';
                parseImgs(product.image).forEach((url,i) => {
                    imgs.push({
                        ...
                        category: 'Thu hoạch',
                        date:     batch?.harvestdate ? new Date(batch.harvestdate).toLocaleDateString('vi-VN') : '',
                        farmName,
    ...
    const filtered = images.filter(img =>
        ...
        (!search || img.title.toLowerCase().includes(search.toLowerCase()) || (img.farmName||'').toLowerCase().includes(search.toLowerCase()))
    );
        ...
        <div className="space-y-5">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div className="flex gap-2">
                    ...
                    <button onClick={()=>inputRef.current?.click()} disabled={uploading}
                        ...
                        {uploading ? <><div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin"/>Đang upload...</>
                            : uploaded ? <><Check size={15}/>Đã upload!</>
            ...
            <div className="flex flex-col md:flex-row gap-3">
                <div className="relative flex-1 max-w-xs">
                    ...
                    <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Tìm ảnh, tên vườn..."
            ...
            <div className="flex gap-4 text-xs" style={{ color:'rgba(255,255,255,0.3)' }}>
                <span className="font-bold text-white">{filtered.length}</span> ảnh
                <span>·</span>
                <span><span className="font-bold text-white">{likes}</span> yêu thích</span>
                <span>·</span>
                <span>Farm → Batch → Product</span>
            </div>
            ...
            {loading ? (
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                    {filtered.map(img => {
                        ...
                        return (
                            <div key={img.id} className="group relative aspect-[4/3] rounded-2xl overflow-hidden cursor-pointer transition-all hover:-translate-y-0.5 hover:shadow-2xl"
                    ...
                    <div onClick={()=>inputRef.current?.click()}
                         className="aspect-[4/3] rounded-2xl flex flex-col items-center justify-center cursor-pointer transition-all group"
                         style={{ border:'2px dashed rgba(255,255,255,0.08)' }}
                         onMouseEnter={e=>e.currentTarget.style.borderColor='rgba(34,197,94,0.3)'}
                         onMouseLeave={e=>e.currentTarget.style.borderColor='rgba(255,255,255,0.08)'}>
                        <Upload size={22} className="mb-2 transition-colors" style={{ color:'rgba(255,255,255,0.2)' }}/>
                        <p className="text-xs font-semibold transition-colors" style={{ color:'rgba(255,255,255,0.2)' }}>Thêm ảnh</p>
                    </div>
                </div>
            )}
